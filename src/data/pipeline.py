...
def _normalise_columns(frame:pd.DataFrame)->pd.DataFrame:
    ...
    aliases={"time":"timestamp","time_stamp":"timestamp","junction_name":"junction","total_vehicle":"total_vehicles","precipitation":"precipitation_mm"}
...
def load_traffic(path:Path)->pd.DataFrame:
    ...
    frame["timestamp"]=pd.to_datetime(frame["timestamp"],errors="coerce",utc=True,format="mixed"); frame["junction"]=frame["junction"].astype(str).str.strip()
...
def _regularize_time_grid(frame:pd.DataFrame,config:TensorConfig)->pd.DataFrame:
    """Collapse timestamp jitter and create a complete 5-minute node grid."""
    frame=frame.copy()
    interval=f"{config.resample_minutes}min"
    frame["timestamp"]=pd.to_datetime(frame["timestamp"],utc=True,format="mixed").dt.floor(interval)
    junctions=sorted(frame["junction"].astype(str).unique())
    numeric=[column for column in FEATURE_NAMES if column in frame]
    grouped=frame.groupby(["timestamp","junction"],as_index=False,sort=True)[numeric].mean()
    if grouped.empty:
        raise ValueError("traffic data has no usable rows after timestamp parsing")
    timestamps=pd.date_range(grouped["timestamp"].min(),grouped["timestamp"].max(),freq=interval,tz="UTC")
    index=pd.MultiIndex.from_product([timestamps,junctions],names=["timestamp","junction"])
    return grouped.set_index(["timestamp","junction"]).reindex(index).reset_index()

def make_windows(frame:pd.DataFrame,config:TensorConfig,adjacency:np.ndarray|None=None)->dict:
    frame=_regularize_time_grid(frame,config); frame=impute_features(frame,adjacency,config); frame=frame.sort_values(["timestamp","junction"]); junctions=sorted(frame["junction"].unique()); timestamps=sorted(frame["timestamp"].unique()); train_end,validation_end=_split_dates(pd.DatetimeIndex(timestamps),config)
